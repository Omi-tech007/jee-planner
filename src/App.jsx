...
import { 
  ...
  Tag, Menu, HelpCircle, MessageSquare, Send, Bot
} from 'lucide-react';
import { 
...
/**
 * PREPPILOT - v37.0 (AI Tutor Chat + Context Aware)
 */
    ...
    <div className="w-full overflow-x-auto pb-2 no-scrollbar">
      <div className="flex flex-col gap-1 min-w-[600px]">
         <div className="grid grid-rows-7 grid-flow-col gap-1 h-[100px]">
            {data.map((day) => (
              <div key={day.date} title={`${day.date}: ${Math.round((history[day.date]||0)/60)}h`} className={`w-3 h-3 rounded-sm transition-all hover:scale-125 ${getCellColor(day.intensity)}`} />
            ))}
         </div>
...
// --- AI TUTOR COMPONENT (NEW) ---
const AITutorCard = ({ data, theme, isDark }) => {
  const [messages, setMessages] = useState([{ role: 'model', text: 'Hello Pilot! I am your AI Co-Pilot. Ask me to explain a concept, solve a doubt, or analyze your study data.' }]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const context = `
        SYSTEM INSTRUCTION: You are PrepPilot, an expert JEE/NEET tutor.
        USER CONTEXT:
        - Target Exams: ${data.selectedExams?.join(", ") || "None"}
        - Current Tasks: ${data.tasks.filter(t=>!t.completed).map(t=>t.text).join(", ") || "None"}
        - Study History: ${Object.keys(data.history).length} days recorded.
        - Weakest Subject: (Infer from questions).
        
        GOAL: Answer the user's question accurately. If it's a study doubt (Physics/Math/Chem), solve it step-by-step. If it's about motivation, use their data to encourage them. Be concise but helpful.
      `;
      ...
      setMessages(prev => [...prev, { role: 'model', text: "Radio silence (Error). Please check your internet or API key." }]);
    }
    setLoading(false);
  };

  return (
    <GlassCard isDark={isDark} className="h-[500px] flex flex-col p-0 overflow-hidden">
      <div className={`p-4 border-b ${isDark ? 'border-white/10 bg-white/5' : 'border-gray-200 bg-gray-50'} flex justify-between items-center`}>
        <h3 className={`font-bold text-lg flex items-center gap-2 ${textCol}`}><Bot size={20} className={theme.text} /> AI Tutor</h3>
        <span className="text-[10px] uppercase font-bold text-gray-500 bg-black/10 px-2 py-1 rounded">Gemini 2.5</span>
      </div>
      
      <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
        {messages.map((msg, i) => (
          <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[85%] p-3 rounded-2xl text-sm leading-relaxed whitespace-pre-wrap ${msg.role === 'user' ? `${theme.msgUser} text-white rounded-br-none` : `${isDark ? 'bg-white/10 text-gray-200' : 'bg-gray-100 text-gray-800'} rounded-bl-none`}`}>
              {msg.text}
            </div>
          </div>
        ))}
        {loading && <div className="flex justify-start"><div className={`px-4 py-2 rounded-2xl text-xs animate-pulse ${isDark ? 'bg-white/10 text-gray-400' : 'bg-gray-100 text-gray-500'}`}>Thinking...</div></div>}
        <div ref={bottomRef} />
      </div>

      <div className={`p-4 border-t ${isDark ? 'border-white/10 bg-black/20' : 'border-gray-200 bg-gray-50'} flex gap-2`}>
        <input 
          type="text" 
          value={input}
        ...
        />
        <button onClick={handleSend} disabled={loading} className={`p-2 rounded-full ${theme.bg} text-white transition hover:scale-110 disabled:opacity-50 disabled:scale-100`}>
          <Send size={18} />
        </button>
      </div>
    </GlassCard>
...
const Dashboard = ({ data, setData, goToTimer, user, theme, isDark }) => {
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* NEW AI TUTOR CHAT */}
        <AITutorCard data={data} theme={theme} isDark={isDark} />

        <div className="space-y-6">
            <GlassCard className="h-[240px] flex flex-col" isDark={isDark}>
              <h3 className={`text-lg font-bold ${textCol} mb-4`}>This Week</h3>
              <div className="flex-1 w-full"><ResponsiveContainer width="100%" height="100%"><AreaChart data={getWeeklyData()}><defs><linearGradient id="colorHours" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor={theme.stroke} stopOpacity={0.8}/><stop offset="95%" stopColor={theme.stroke} stopOpacity={0}/></linearGradient></defs><CartesianGrid strokeDasharray="3 3" vertical={false} stroke={isDark ? "rgba(255,255,255,0.05)" : "rgba(0,0,0,0.05)"} /><XAxis dataKey="name" axisLine={false} tickLine={false} tick={{fill: '#9ca3af', fontSize: 12}} /><YAxis hide /><RechartsTooltip contentStyle={{backgroundColor: isDark ? '#18181b' : '#fff', borderColor: isDark ? '#27272a' : '#ddd', color: isDark?'#fff':'#000'}} /><Area type="monotone" dataKey="hours" stroke={theme.stroke} strokeWidth={3} fillOpacity={1} fill="url(#colorHours)" /></AreaChart></ResponsiveContainer></div>
            </GlassCard>
            <GlassCard isDark={isDark} className="h-[240px] flex flex-col">
                <div className="flex justify-between items-center mb-4"><h3 className={`text-lg font-bold ${textCol}`}>Tasks</h3><button onClick={addTask} className={`text-xs px-3 py-1 rounded ${isDark ? 'bg-white/10 text-white' : 'bg-black/10 text-black'}`}>+ Add</button></div>
...
// --- OTHER COMPONENTS (Syllabus, Analysis, Timer, etc. - Kept same as stable version) ---
// Note: Analysis updated to include 12-month calendar heatmap as requested previously.
